(() => {
	function e() {
		...
		return '' === n || 'index' === n.toLowerCase() || 'settings' === n.toLowerCase() || 'signup' === n.toLowerCase() || 'login' === n.toLowerCase() || 'forgot-password' === n.toLowerCase();
	}
	!(function () {
		...
		(e.innerHTML =
			...
			(document.body.innerHTML = e.innerHTML + document.body.innerHTML);
		let n = document.createElement('script');
		(n.type = 'module'), (n.src = 'https://cdn.jsdelivr.net/npm/ldrs/dist/auto/ring.js'), console.log(n), document.head.appendChild(n);
	})(),
		fetch('api/auth/isLoggedIn', { method: 'post', headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
			.then((e) => e.json())
			.then(async (n) => {
				if (n.status)
					if ((n.subscribed || n.admin) && n.isVerified) {
						if (
							((document.querySelector('#username').innerHTML = n.username
								.replace(/&/g, '&amp;')
								.replace(/</g, '&lt;')
								.replace(/>/g, '&gt;')
								.replace(/"/g, '&quot;')
							...
							document.querySelector('#profile').setAttribute('href', `profile?uid=${n.uid}`),
							n.pfp && (document.querySelector('#pfp').src = `/image/${n.pfp}`),
							n.admin)
						) {
							let e = document.createElement('a');
							(e.className = 'navProfileDropLink'),
								(e.innerHTML = '<i class="fa-solid fa-chart-line"></i> Admin Dashboard'),
								(e.href = 'admin'),
								document.querySelector('#navSub').insertAdjacentElement('afterend', e);
						}
					} else e() || (n.isVerified || alert('Your account is not verified. Check your email to verify!'), location.replace('/login'));
				else
					(document.querySelector('#signupNav').innerHTML = '<i id="responsiveNavIcon" class="fa-solid fa-right-to-bracket"></i> Login'),
						...
						e() || (alert('This content requires a Long X Ranch Cowboys subscription!'), location.replace('/login'));
			})
			.catch((e) => {
				console.log(e);
			}),
		document.querySelector('.navIcon').addEventListener('click', function () {
			'flex' == document.querySelector('.nav-links').style.display
				? ((document.querySelector('.nav-links').style.display = 'none'), (document.querySelector('#burgerMenu').innerHTML = '<i class="fa-solid fa-bars"></i>'))
			...
			let e = ((e) => {
				...
				return (
					document.cookie.split(';').forEach((i) => {
						if (i.includes(`${e}=`)) return (n = i.split(`${e}=`)[1]), n;
					}),
					n
				);
			})('pfp');
			(document.querySelector('#pfp').src = e ? `/image/${e}` : '../images/default-pfp.jpeg'),
				fetch('api/profile/getPfp', { method: 'get', headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
					.then((e) => e.json())
					.then(async (e) => {
						e.status ? (document.querySelector('#pfp').src = `/image/${e.pfp}`) : (document.querySelector('#pfp').src = '../images/default-pfp.jpeg');
					})
					.catch((e) => {
						console.log(e);
					});
	...
	window.addEventListener('scroll', () => {
		...
		n.style.backgroundColor = e > 0 ? '#00000085' : 'var(--bistre)';
	});
})();
